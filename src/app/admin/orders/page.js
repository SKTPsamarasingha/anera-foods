...
const ORDER_STATUSES = [
  "Pending", "Confirmed", "Processing", "Ready for Dispatch", "Dispatched", 
...
function OrdersContent() {
  ...
  const getStatusBadgeClass = (status) => {
    switch (status) {
      ...
      case "Ready for Dispatch": return "bg-purple-100 text-purple-800 border-purple-200";
      case "Dispatched": return "bg-orange-100 text-orange-800 border-orange-200";
      case "Delivered": case "Completed": return "bg-green-100 text-green-800 border-green-200";
  ...
  return (
    <div className="grid grid-cols-1 lg:grid-cols-12 gap-8 animate-fade-in font-sans">
      ...
      <div className="lg:col-span-7 bg-white p-6 rounded-2xl border border-gray-100 shadow-sm space-y-6">
        <div>
          <h1 className="text-xl font-bold font-serif text-[#1E3A2F]">Order Management</h1>
          <p className="text-xs text-gray-500">Track and dispatch customer purchases.</p>
        </div>
        ...
        <div className="grid grid-cols-1 sm:grid-cols-2 gap-4">
          <div className="relative">
            <input
              ...
              className="w-full pl-9 pr-4 py-2.5 text-xs border border-gray-200 rounded-lg focus:outline-none focus:border-[#C27D38]"
            />
          ...
          <div className="flex items-center gap-2">
            <select
              ...
              className="w-full p-2.5 border border-gray-200 rounded-lg text-xs focus:outline-none focus:border-[#C27D38] bg-white text-gray-600"
            >
                ...
                filteredOrders.map(o => (
                  <tr 
                    ...
                    className={`cursor-pointer hover:bg-gray-50 transition-colors ${
                      selectedOrder?.id === o.id ? "bg-[#1E3A2F]/5 font-semibold" : ""
                    }`}
                  >
                    <td className="py-3.5 text-[#1E3A2F] font-bold">{o.orderNumber}</td>
                    <td className="py-3.5">
                      <span className="block text-gray-800">{o.customer.name}</span>
                      <span className="block text-[10px] text-gray-400 font-normal">{new Date(o.createdAt).toLocaleDateString()}</span>
                    </td>
                    <td className="py-3.5 text-gray-500">{o.customer.district}</td>
                    <td className="py-3.5 font-bold text-gray-700">LKR {o.totalAmount.toLocaleString()}</td>
                    <td className="py-3.5">
                      <span className={`px-2 py-0.5 rounded-full border text-[9px] font-bold ${getStatusBadgeClass(o.status)}`}>
                        {o.status}
                      </span>
                    </td>
                  </tr>
        ...
        {selectedOrder ? (
          <div className="bg-white p-6 rounded-2xl border border-gray-100 shadow-sm space-y-6 animate-fade-in sticky top-24">
            ...
            <div className="flex justify-between items-start border-b border-gray-100 pb-3">
              <div>
                <h2 className="text-lg font-bold font-serif text-[#1E3A2F]">{selectedOrder.orderNumber}</h2>
                <span className="text-[10px] text-gray-400 font-sans">{new Date(selectedOrder.createdAt).toLocaleString()}</span>
              </div>
              
              {/* Status Update drop down */}
              <div className="space-y-1">
                <label className="text-[9px] uppercase font-bold text-gray-400 block font-sans text-right">Process Status</label>
                <select
                  value={selectedOrder.status}
                  onChange={(e) => handleStatusChange(selectedOrder.id, e.target.value)}
                  className="p-1 border border-gray-200 rounded text-xs focus:outline-none focus:border-[#C27D38] bg-white font-semibold text-[#1E3A2F]"
                >
                  {ORDER_STATUSES.map(s => (
                    <option key={s} value={s}>{s}</option>
                  ))}
            ...
            {previewInvoice ? (
              ...
              <div className="space-y-4">
                <div className="flex justify-between items-center">
                  <span className="text-xs font-bold font-serif text-[#1E3A2F]">Invoice Packing Slip</span>
                  <button 
                ...
                <div 
                  ...
                  dangerouslySetInnerHTML={{ __html: generateInvoiceHtml(selectedOrder) }}
                />
                <button
              ...
              <div className="space-y-5 text-xs font-sans">
                ...
                {selectedOrder.status !== "Completed" && selectedOrder.status !== "Cancelled" && (
                  <div className="bg-[#F0F4F1] p-4 rounded-xl border border-[#1E3A2F]/5 space-y-2">
                    <span className="text-[10px] uppercase font-bold text-gray-400 block font-sans">Next Action Step</span>
                ...
                <div className="space-y-2">
                  <h3 className="text-xs uppercase font-bold text-[#C27D38] tracking-wider font-sans">Buyer Details</h3>
                  <div className="bg-gray-50 p-4 rounded-xl space-y-1.5 text-gray-600">
                    <p><strong>Name:</strong> {selectedOrder.customer.name}</p>
                    <p>
                      <strong>Phone:</strong>{" "}
                      <a href={`tel:${selectedOrder.customer.phone}`} className="text-[#C27D38] font-bold hover:underline">
                        {selectedOrder.customer.phone}
                      </a>
                    </p>
                    <p><strong>Email:</strong> {selectedOrder.customer.email}</p>
                    <p><strong>Address:</strong> {selectedOrder.customer.address}, {selectedOrder.customer.city}, {selectedOrder.customer.district}</p>
                  </div>
                </div>

                {/* Items Purchased */}
                <div className="space-y-2">
                  <h3 className="text-xs uppercase font-bold text-[#C27D38] tracking-wider font-sans">Items Purchased</h3>
                  <div className="divide-y divide-gray-100">
                    {selectedOrder.items.map((item, idx) => (
                      <div key={idx} className="py-2.5 flex justify-between">
                        <div>
                          <span className="font-semibold text-gray-800">{item.name}</span>
                          <span className="block text-[10px] text-gray-400">Qty: {item.quantity} • {item.weight}</span>
                        </div>
                        <span className="font-semibold text-gray-700">LKR {(item.price * item.quantity).toLocaleString()}</span>
                      </div>
                    ))}
                  </div>
                </div>
                ...
                <div className="border-t border-gray-100 pt-3 space-y-1.5 text-gray-500">
                  ...
                  <div className="flex justify-between border-t border-gray-100 pt-2 text-[#1E3A2F] font-bold">
                    ...
                    <span className="text-[#C27D38]">LKR {selectedOrder.totalAmount.toLocaleString()}</span>
                  </div>
                </div>
                ...
                {selectedOrder.notes && (
                  <div className="bg-[#FDFBF7] p-3 rounded-lg border border-dashed border-[#1E3A2F]/10">
                    <span className="text-[10px] uppercase font-bold text-gray-400 block mb-1">Customer Notes:</span>
          ...
          <div className="bg-white p-12 rounded-2xl border border-gray-100 shadow-sm text-center text-gray-400 text-xs py-20 font-sans">
            ...
            Select an order from the list to view billing information, update shipping status, or preview print slips.
          </div>
        )}
      </div>
