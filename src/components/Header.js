...
export default function Header() {
  ...
  return (
    <>
      <header className="sticky top-0 z-40 w-full bg-[#FDFBF7]/90 backdrop-blur-md border-b border-[#1E3A2F]/10 shadow-sm transition-all duration-300">
        <div className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8">
          <div className="flex justify-between items-center h-20">
            {/* Logo */}
            <div className="flex-shrink-0">
              <Link href="/" className="flex flex-col items-start leading-none group">
                <span className="text-2xl sm:text-3xl font-extrabold text-[#1E3A2F] font-serif tracking-wider transition-colors group-hover:text-[#C27D38]">
                  ANERA FOODS
                </span>
                <span className="text-[9px] sm:text-[10px] tracking-[0.25em] text-[#C27D38] font-semibold uppercase mt-0.5">
                  Premium Sri Lanka
                </span>
              </Link>
            </div>
            ...
            <nav className="hidden md:flex space-x-8">
              <Link
                ...
                className={`text-sm font-semibold tracking-wide uppercase transition-colors duration-200 ${
                  isActive("/") ? "text-[#C27D38]" : "text-[#1E3A2F]/80 hover:text-[#1E3A2F]"
                }`}
              >
                Home
              </Link>
              <Link
                href="/products"
                className={`text-sm font-semibold tracking-wide uppercase transition-colors duration-200 ${
                  isActive("/products") ? "text-[#C27D38]" : "text-[#1E3A2F]/80 hover:text-[#1E3A2F]"
                }`}
              >
                Products
              </Link>
              <Link
                href="/about"
                className={`text-sm font-semibold tracking-wide uppercase transition-colors duration-200 ${
                  isActive("/about") ? "text-[#C27D38]" : "text-[#1E3A2F]/80 hover:text-[#1E3A2F]"
                }`}
              >
                Our Story
              </Link>
              <Link
                href="/track"
                className={`text-sm font-semibold tracking-wide uppercase transition-colors duration-200 ${
                  isActive("/track") ? "text-[#C27D38]" : "text-[#1E3A2F]/80 hover:text-[#1E3A2F]"
                }`}
              >
                Track Order
              </Link>
              ...
              <Link
                href="/contact"
                className={`text-sm font-semibold tracking-wide uppercase transition-colors duration-200 ${
                  isActive("/contact") ? "text-[#C27D38]" : "text-[#1E3A2F]/80 hover:text-[#1E3A2F]"
                }`}
              >
                Contact
              </Link>
            ...
            <div className="flex items-center gap-3">
                  ...
                  {isAuthenticated ? (
                    <div className="relative">
                      <button
                        ...
                        className="flex items-center gap-2 text-sm font-semibold text-[#1E3A2F]/80 hover:text-[#1E3A2F] transition-colors group"
                      >
                        <span className="w-8 h-8 rounded-full bg-gradient-to-br from-[#1E3A2F] to-[#C27D38] flex items-center justify-center text-white text-xs font-bold uppercase shadow-md group-hover:shadow-lg transition-shadow">
                          {user?.name?.charAt(0) || "U"}
                        </span>
                        <span className="hidden sm:inline font-sans">{user?.name?.split(" ")[0]}</span>
                        ...
                        <>
                          ...
                          <div className="absolute right-0 mt-2 w-56 bg-white rounded-xl shadow-2xl border border-gray-100 py-2 z-40 animate-fade-in">
                            <div className="px-4 py-3 border-b border-gray-100">
                              <p className="text-sm font-bold text-[#1E3A2F] font-sans">{user?.name}</p>
                    ...
                    <div className="hidden sm:flex items-center gap-2">
                      <Link
                        href="/login"
                        className="text-sm font-semibold text-[#1E3A2F]/70 hover:text-[#1E3A2F] transition-colors font-sans px-3 py-1.5"
                      >
                        Sign In
                      </Link>
                      <Link
              ...
              <button
                ...
                className="relative p-2.5 rounded-full hover:bg-[#1E3A2F]/5 text-[#1E3A2F] transition-all group"
      ...
      {isCartOpen && (
        <div className="fixed inset-0 z-50 flex justify-end">
          ...
          <div className="relative w-full max-w-md bg-[#FDFBF7] h-full shadow-2xl flex flex-col z-10 animate-fade-in border-l border-[#1E3A2F]/10">
            <div className="p-6 border-b border-[#1E3A2F]/10 flex justify-between items-center bg-[#1E3A2F] text-white">
            ...
            <div className="flex-1 overflow-y-auto p-6 space-y-4">
              {!isMounted || cartItems.length === 0 ? (
                <div className="flex flex-col items-center justify-center h-64 text-center">
                  ...
                  <p className="text-gray-500 font-sans font-medium">Your cart is empty</p>
                  <Link
                    href="/products"
                    onClick={() => setIsCartOpen(false)}
                ...
                cartItems.map((item) => (
                  <div key={item.id} className="flex gap-4 p-3 rounded-lg bg-white border border-gray-100 shadow-sm relative group">
                    <div className="w-16 h-16 bg-gray-50 rounded overflow-hidden flex-shrink-0 relative">
                      {item.images && item.images[0] ? (
                        ...
                        <div className="w-full h-full bg-[#1E3A2F]/5 flex items-center justify-center font-bold text-xs text-[#1E3A2F]">
                    ...
                    <div className="flex-1 flex flex-col justify-between">
                      <div>
                        <h4 className="text-sm font-semibold text-[#1E3A2F] line-clamp-1">{item.name}</h4>
                      ...
                      <div className="flex justify-between items-center mt-2">
                        ...
                        <div className="flex items-center border border-gray-200 rounded-md">
                          <button
                            ...
                            className="px-2 py-0.5 text-gray-500 hover:text-red-500 transition-colors"
                          >
                            -
                          </button>
                          ...
                          <button
                            onClick={() => updateQuantity(item.id, item.quantity + 1)}
                            className="px-2 py-0.5 text-gray-500 hover:text-green-500 transition-colors"
                          >
                            +
                          </button>
                        </div>
                        <span className="text-xs font-bold text-[#C27D38]">LKR {(item.price * item.quantity).toLocaleString()}</span>
                      </div>
                    </div>
            ...
            {isMounted && cartItems.length > 0 && (
              <div className="p-6 border-t border-[#1E3A2F]/10 bg-white space-y-4">
                <div className="flex justify-between items-center">
                  <span className="text-sm text-gray-500 font-medium font-sans">Subtotal</span>
                  <span className="text-lg font-bold text-[#1E3A2F]">LKR {cartSubtotal.toLocaleString()}</span>
                </div>
                <p className="text-[11px] text-gray-400 text-center font-sans">Shipping rates calculated during checkout.</p>
                <div className="grid grid-cols-2 gap-3">
                  <button
                    onClick={() => setIsCartOpen(false)}
                    className="btn btn-outline py-2.5 text-xs text-center"
                  >
                    Keep Browsing
                  </button>
                  <Link
